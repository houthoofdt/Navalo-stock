    ...
    if (storage.getMode() === 'googlesheets') {
        try {
            ...
            if (Array.isArray(remoteInvoices) && remoteInvoices.length > 0) {
                ...
                const remoteById = new Map(remoteInvoices.map(i => [i.id, i]));
                const remoteByNumber = new Map(remoteInvoices.map(i => [i.internalNumber, i]));
                
                // Merge: update local with remote data, keep local-only items with fileData
                const merged = [];
                const processedIds = new Set();
                
                // First, process remote invoices
                for (const remote of remoteInvoices) {
                    // Find matching local invoice (by ID or internalNumber)
                    const local = invoices.find(l => l.id === remote.id || l.internalNumber === remote.internalNumber);
                    
                    if (local) {
                        // Merge: keep local fileData, use remote for other fields
                        merged.push({
                            ...remote,
                            fileData: local.fileData || remote.fileData,
                            fileName: local.fileName || remote.fileName,
                            fileType: local.fileType || remote.fileType,
                            driveFileId: local.driveFileId || remote.driveFileId,
                            driveFileUrl: local.driveFileUrl || remote.driveFileUrl,
                            varSymbol: local.varSymbol || remote.varSymbol,
                            supplierIco: local.supplierIco,
                            supplierDic: local.supplierDic
                        });
                        processedIds.add(local.id);
                        processedIds.add(local.internalNumber);
                    } else {
                        merged.push(remote);
                    }
                    processedIds.add(remote.id);
                    processedIds.add(remote.internalNumber);
                }
                
                // Then, add local-only invoices (not yet synced)
                for (const local of invoices) {
                    if (!processedIds.has(local.id) && !processedIds.has(local.internalNumber)) {
                        merged.push(local);
                    }
                }
