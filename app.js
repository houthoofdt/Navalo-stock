...
function getNextRecOrderNumber(consume = false) {
    const config = JSON.parse(localStorage.getItem('navalo_config') || '{}');
    const year = new Date().getFullYear();
...
function openReceivedOrderModal() {
    ...
    document.getElementById('recOrdNumber').value = getNextRecOrderNumber(false); // Preview only
...
async function saveReceivedOrder() {
    ...
    if (!editingRecOrderId) {
        ...
        orderNumber = getNextRecOrderNumber(true);
