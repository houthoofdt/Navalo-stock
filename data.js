...
function getComponentPrice(ref, currency) {
    ...
    if (typeof loadedComponentPrices !== 'undefined' && loadedComponentPrices[ref]) {
        const loaded = loadedComponentPrices[ref];
        let price = null;
        if (currency === 'EUR') {
            price = loaded.eur || loaded.priceEur || null;
        } else if (currency === 'CZK') {
            price = loaded.czk || loaded.priceCzk || null;
        } else {
            price = loaded.eur || loaded.priceEur || loaded.czk || loaded.priceCzk || null;
        }
        // Ensure we return a number
        return price !== null ? parseFloat(price) || null : null;
    }
    
    // Fallback to static prices
    const prices = COMPONENT_PRICES[ref];
    ...
    if (currency === 'EUR') {
        ...
        price = prices.eur || prices.czk;
    }
    return price !== null && price !== undefined ? parseFloat(price) || null : null;
}
